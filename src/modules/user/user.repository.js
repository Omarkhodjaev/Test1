...
class UserRepository extends Postgress {
  ...
  async getAllUsers() {
    return await this.fetchAll(
      "SELECT u.id, u.login, u.password, u.full_name, u.birthdate, row_to_json(f) AS file  FROM users u  INNER JOIN files f ON u.file_id = f.id;"
    );
  }

  async insertUser(UserEntity) {
    return await this.fetch(
      "INSERT INTO users(login, password, full_name, birthdate, role, file_id) VALUES($1, $2, $3, $4, $5, $6) returning *",
      UserEntity.login,
      UserEntity.password,
  ...
  async update(updatedUser, userId) {
    return await this.fetch(
      `UPDATE users SET login = $1, password = $2, full_name = $3, birthdate = $4 WHERE id = $5 RETURNING *`,
      updatedUser.login,
